...
def rescale_volume(
    ...
    new_volume = volume.copy()
    intensities = (
        new_volume[new_volume > 0]
        if use_positive_only
        else new_volume.flatten()
    )
...
def crop_volume(
    ...
    if cropping_margin is not None:
        ...
        do_cropping = np.array(vol_shape[:n_dims]) > 2 * np.array(
            cropping_margin
        )
        min_crop_idx = [
        ...
        if mode == "center":
            ...
            max_crop_idx = np.minimum(
                min_crop_idx + np.array(cropping_shape),
                np.array(vol_shape)[:n_dims],
            )
        else:
            raise ValueError(
...
def crop_volume_around_region(
    ...
    if np.any(mask):
        ...
        if pad_margins is not None:
            ...
            new_vol = np.pad(
                new_vol, pad_margins, mode="constant", constant_values=0
            )
...
def pad_volume(
    volume, padding_shape, padding_value=0, aff=None, return_pad_idx=False
):
    ...
    vol_shape = new_volume.shape
    n_dims, n_channels = utils.get_dims(vol_shape)
    padding_shape = utils.reformat_to_list(
        padding_shape, length=n_dims, dtype="int"
    )
    ...
    if np.any(
        ...
        pad_margins = tuple(
            [(min_margins[i], max_margins[i]) for i in range(n_dims)]
        )
        if n_channels > 1:
        ...
        pad_idx = np.concatenate(
            [np.array([0] * n_dims), np.array(vol_shape[:n_dims])]
        )
...
def resample_volume_like(
    vol_ref, aff_ref, vol_flo, aff_flo, interpolation="linear"
):
...
def align_volume_to_ref(
    volume, aff, aff_ref=None, return_aff=False, n_dims=None
):
    ...
    for i in range(n_dims):
        if ras_axes_flo[i] != ras_axes_ref[i]:
            new_volume = np.swapaxes(
                new_volume, ras_axes_flo[i], ras_axes_ref[i]
            )
...
def correct_label_map(
    ...
    ) | use_nearest_label, "please provide a list of correct labels, or set use_nearest_label to True."
...
def compute_distance_map(labels, masking_labels=None, crop_margin=None):
    ...
    if crop_margin is not None:
        tmp_labels, crop_idx = crop_volume_around_region(
            labels, margin=crop_margin
        )
    else:
        tmp_labels = labels
        crop_idx = None
...
def crop_images_in_dir(
    ...
    for idx, path_image in enumerate(path_images):
        ...
        if (not os.path.isfile(path_result)) | recompute:
            ...
            volume, aff = crop_volume(
                volume, cropping_margin, cropping_shape, aff
            )
    ...
    for idx, (path_image, path_ref) in enumerate(zip(path_images, path_refs)):
        ...
        if (not os.path.isfile(path_result)) | recompute:
            ...
            im, aff = align_volume_to_ref(
                im, aff, aff_ref=aff_ref, return_aff=True
            )
            utils.save_volume(im, aff, h, path_result)
...
def convert_images_in_dir_to_nifty(
    ...
    for idx, (path_image, path_ref) in enumerate(
        zip(path_images, path_ref_images)
    ):
        loop_info.update(idx)
...
def niftyreg_images_in_dir(
    ...
    if same_floating:
        path_images = utils.reformat_to_list(
            path_images, length=len(path_references)
        )
    assert len(path_references) == len(
        path_images
    ), "different number of files in image_dir and reference_dir"
    ...
    for idx, (path_image, path_ref, path_input_trans) in enumerate(
        ...
        if result_transformation_dir is not None:
            if nifty_reg_function == "reg_aladin":
                ...
                path_result_trans = os.path.join(
                    result_transformation_dir, name
                )
                result_trans_already_computed = os.path.isfile(
                    path_result_trans
                )
        ...
        if (
            ...
            # build main command
            cmd = utils.mkcmd(
                nifty_reg, "-ref", path_ref, "-flo", path_image, "-pad 0"
            )
...
def upsample_anisotropic_images(
    ...
    for idx, (path_image, path_ref) in enumerate(
        ...
        if (not os.path.isfile(path_dist_map)) | recompute:
            im, aff, h = utils.load_volume(path_image, im_only=False)
            dist_map = np.meshgrid(
                *[np.arange(s) for s in im.shape], indexing="ij"
            )
            ...
            mesh_up_0, aff, h = utils.load_volume(
                path_meshes_up[0], im_only=False
            )
            mesh_up = np.stack(
                [mesh_up_0]
                + [utils.load_volume(p) for p in path_meshes_up[1:]],
                -1,
            )
...
def check_images_in_dir(
    ...
    for idx, path_image in enumerate(path_images):
        ...
        aff = (
            np.int32(np.round(np.array(aff[:3, :3]), 2) * 100) / 100
        ).tolist()
...
def mask_labels_in_dir(
    ...
    for idx, path_label in enumerate(path_labels):
        ...
        if (
            ...
            | (mask_result_dir is not None)
            & (not os.path.isfile(path_result_mask))
            | recompute
        ):
...
def smooth_labels_in_dir(
    ...
    if labels_list is not None:
        labels_list, _ = utils.get_list_labels(
            label_list=labels_list, FS_sort=True
        )
    ...
    if gpu:
        ...
        for idx, path_label in enumerate(path_labels):
            ...
            if (not os.path.isfile(path_result)) | recompute:
                (
                    ...
                    h,
                    _,
                ) = utils.get_volume_info(path_label, return_volume=True)
        ...
        kernel = utils.build_binary_structure(
            connectivity, n_dims, shape=n_dims
        )
        ...
        for idx, path in enumerate(path_labels):
            ...
            if (not os.path.isfile(path_result)) | recompute:
                ...
                utils.save_volume(
                    new_volume, aff, h, path_result, dtype="int32"
                )
...
def upsample_labels_in_dir(
    ...
    post_cmd = (
        "-voxsize " + " ".join([str(r) for r in target_res]) + " -odt float"
    )
        ...
        if (not os.path.isfile(path_result)) | recompute:
            ...
            for label in new_label_list:
                path_mask = os.path.join(
                    indiv_label_dir, str(label) + ".nii.gz"
                )
            ...
            probmax, aff, h = utils.load_volume(
                os.path.join(upsample_indiv_label_dir, "0.nii.gz"),
                im_only=False,
            )
            ...
            utils.save_volume(
                label_list[labels], aff, h, path_result, dtype="int32"
            )
...
def compute_hard_volumes_in_dir(
    ...
    label_list, _ = utils.get_list_labels(
        path_label_list, labels_dir, FS_sort=FS_sort
    )
...
def check_images_and_labels(image_dir, labels_dir):
    ...
    loop_info = utils.LoopInfo(len(path_images), 10, "checking", True)
    for idx, (path_image, path_label) in enumerate(
...
def crop_dataset_to_minimum_size(
    ...
    for (path_label, path_image) in tqdm(zip(path_labels, path_images)):
        # loop_info.update(idx)
    ...
    for (path_label, path_image) in tqdm(zip(path_labels, path_images)):
        ...
        if path_image is not None:
            path_result = os.path.join(
                image_result_dir, os.path.basename(path_image)
            )
...
def crop_dataset_around_region_of_same_size(
    ...
    for path_label, path_image in zip(path_labels, path_images):
        ...
        path_label_result = os.path.join(
            result_dir, os.path.basename(path_label)
        )
        path_image_result = os.path.join(
            image_result_dir, os.path.basename(path_image)
        )
        ...
        if (
            ...
            if pad_margins is not None:
                label = np.pad(
                    label, pad_margins, mode="constant", constant_values=0
                )
                if path_image is not None:
            ...
            utils.save_volume(
                label, aff_final, h_la, path_label_result, dtype="int32"
            )
            if path_image is not None:
...
def crop_dataset_around_region(
    ...
    for idx, (path_image, path_label) in enumerate(
        ...
        if (
            ...
            utils.save_volume(
                label, aff, h_lab, path_label_result, dtype="int32"
            )
